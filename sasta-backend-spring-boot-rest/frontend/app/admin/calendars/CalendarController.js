app.controller('CalendarController',['$http','$window','$scope','$rootScope','notify','$location','$state','storage',
    function($http,$window,$scope,$rootScope,notify,$location,$state,storage){
        ...
        $scope.manageCalender = function(){
            $scope.gridOptions = {
                ...
                save: function (e) {
                    ...
                    e.model.createdBy =  $rootScope.sessionConfig.userId;
                    e.model.modifiedBy = $rootScope.sessionConfig.userId;
                    e.model.calenderId = parseInt($scope.calendarid,10);
                ...
                dataSource : {
                    transport: {
                        read:  function (e) {
                            $http({
                                method: 'GET',
                                url: $scope.crudServiceBaseUrl + '/calender/detailedsastacalender?id='+($scope.calendarid||''),
                                cache:false
                            }).
                            success(function(data, status, headers, config) {
                                    var ds = [];
        ...
        if($location.search().mode === 'list'){
            $scope.mode = 1;
            $scope.pageTitle = "Calendars"
            $scope.bindCalenders();
        }else if($location.search().mode === 'add'){
            $scope.mode = 2;
            $scope.pageTitle = "Add Calendars";
            $scope.calendarid = $location.search().calenderid;
            $scope.manageCalender();
        }else if($location.search().mode === 'edit'){
            $scope.mode = 3;
            $scope.calendarid = $location.search().calendarid;
            $scope.pageTitle = "Edit Calendars";
            $scope.manageCalender();
        }
